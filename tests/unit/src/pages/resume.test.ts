...
vi.mock("astro:content", async () => ({
	getCollection: async (collectionName: string): Promise<ResumeCollection[]> => {
...
describe("Resume Page", () => {
	it("should load work experience content", async () => {
		const resumeCollection = (await getCollection("resume")) as ResumeCollection[];
		expect(resumeCollection).toHaveLength(1);
		expect(resumeCollection[0].data.work_experience[0].position).toBe("Developer");
	});
});
